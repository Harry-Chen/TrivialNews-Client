...
import kotlinx.android.synthetic.main.news_detail_panel.*
...
class NewsDetailActivity : AppCompatActivity() {
    ...
    private fun setupSlidePanel() {
        detail_sliding_layout.addPanelSlideListener(object:SlidingUpPanelLayout.PanelSlideListener{
            override fun onPanelSlide(panel: View?,
                                      slideOffset: Float) {
                detail_panel_arrow.rotation = if (slideOffset >= 0.7) 180f
    ...
    private fun loadNews() {
        ...
        supportFragmentManager.beginTransaction().replace(R.id.detail_web, webFragment).commit()
